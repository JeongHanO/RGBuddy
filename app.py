...
def calendar():
    ...
    if login[0] == True:
        ...
        enMonth = ['', 'January', 'February', 'March', 'April', 'May', 'June',
                   'July', 'August', 'September', 'October', 'November', 'December']
        weekrange = []
        for i in range(14):
            weekrange.append((now-timedelta(days=weekday-i)).day)
        print(enMonth[month])
        return render_template('calendar_.html', year=year, month=month, enMonth=enMonth[month], day=day, weekday=weekday, weekrange=weekrange)
    else:
        return login[1]
...
def matching():
    ...
    if login[0] == True:
        ...
        enDate = str(year) + " "+enMonth[month]+" "+str(day)
...
def matchLog():
    ...
    if login[0] == True:
        ...
        for datadate in dates:
            data = db.dates.find_one({'date': datadate})
            index = data[userInfo['team']].index(userInfo['id'])

            year, month, day = map(str, data['date'].split("/"))
            print(index)
            if ((date.today()-date(int(year), int(month), int(day))).days < 0):
                if min(len(data['red']), len(data['green']), len(data['blue'])) < index+1:
                   # 매칭 되지 않은 날짜
                    not_matched.append(datadate)
                else:
                   # 매칭 된 날짜
                    ...
                    for team in ['red', 'green', 'blue']:
                        ...
                        name = db.users.find_one(
                            {'id': data[team][index]})['name']
                        phone = db.users.find_one(
                            {'id': data[team][index]})['phone']
                        suc_date.append([name, phone])
...
def api_apply():
    ...
    year, month, day = map(str, date_receive.split(" "))
    enMonth = ['', 'January', 'February', 'March', 'April', 'May', 'June',
               'July', 'August', 'September', 'October', 'November', 'December']
    month_number = enMonth.index(month)
    date_formatted = str(year)+'/'+str(month_number)+'/'+str(day)
    ...
    if data is not None:
        ...
        print(new_list)
        db.dates.update_one({'date': date_formatted}, {
                            "$set": {teamColor: new_list}})
